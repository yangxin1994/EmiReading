 ...
 * @description : 写入插件
 * @company :翼迈科技
 * @date 2018年07月30日下午 02:12
 * @Email: 971613168@qq.com
 */

public class LogFileLogPlugin extends BaseLogPlugin {
    ...
    public LogFileLogPlugin(Context mContext, String mDirectory) {
    ...
    private void saveMessageToSDCard(int type, String tag, String message) {
        ...
        File dir = new File(ROOT_PATH + mDirectory);
        if (!dir.exists()) {
            dir.mkdirs();
        }
        ...
        mLogFile = new File(ROOT_PATH + mDirectory + File.separator + fileName + FILE_NAME_SUFFIX);
        try {
            if (!mLogFile.exists()) {
